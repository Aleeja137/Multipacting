...
unsigned long long int pasos_simulados, aux_pasos_simulados=0;
...
void set_parameters_dictionary(){
    ...
    parallel             = stoi(p["parallel"]);
    std::cout << "Parallel type: " << parallel << std::endl;
...
std::tuple<int, int> run_n_electrons_parallel (double power, std::vector<int> pool_runs, std::vector<double> pool_phase, std::vector<double> pool_energies){
    ...
    while (!ended)
    {
        ...
        // Se crean listas idénticas a las de entrada para las siguientes generaciones
        ...
        aux_pasos_simulados = pasos_simulados;
        ...
        for (int i=0;i<pool_runs.size();i++)
        {
            ...
            if (face != 0) 
            {
                ...
                for (int i=0;i<n_e;i++)
                {
                    ...
                    new_pool_energies.push_back(energies[i]);
                }
                
                // Si se debe monitorizar la ejecución, se guarda en fichero la información de la colisión y los electrones secundarios
                if (log_exec)
                {
                    std::ofstream logfile(logfile_name, std::ios::app);
        ...
        aux_pasos_simulados = pasos_simulados-aux_pasos_simulados;
        // Una vez simulados todos los electrones de la generación, se toma la siguiente generación como la actual
        ...
        std::cout << "Pasos simulador run: " << aux_pasos_simulados << std::endl;
 ...
 * Esta función parte de una lista que contiene uno o varios electrones, y sus listas complementarias que contienen las energías y fases iniciales de cada electrón.
 * Despues, simula los electrones iniciales y añade los electrones resultantes por la colisión a la siguiente 'generación'. Repite este proceso hasta alcanzar el número
 * de generaciones máximo. La simulación de los electrones de cada generación se distribuye entre todos los threads disponibles según scheduling guiado. Esta función es igual
 * que run_nelectrons_parallel usando threads para acelerar la ejecución
 *
 * @param power double que representa la fuerza del dispositivo representado con el mallado
 * @param pool_runs lista de integers que contiene las caras por las cuales se lanzan los electrones a simular
 * @param pool_phase lista de doubles que contiene las fases iniciales de los electrones a simulas 
 ...
 * @return std::tuple<int, int> Representan la cantidad de electrones que se han simulado en total y la cantidad de electrones que se han simulado en la última generación.
 */
std::tuple<int, int> run_n_electrons_parallel_thread (double power, std::vector<int> pool_runs, std::vector<double> pool_phase, std::vector<double> pool_energies){
    ...
    #pragma omp parallel
    {
        ...
        // Se crean listas idénticas a las de entrada para las siguientes generaciones
        std::vector<int> private_new_pool_runs;
        std::vector<double> private_new_pool_phases, private_new_pool_energies;
        ...
        while (!ended)
        {
            ...
            {   
                ...
                if (face != 0) 
                {
                    ...
                    for (int j=0;j<n_e;j++)
                    {
                        private_new_pool_runs.push_back(face);
                        private_new_pool_phases.push_back(phase);
                        private_new_pool_energies.push_back(energies[j]);
                    }
                }               
            }

            // Uno a uno se reúnen las partes de la siguiente generación
            #pragma omp critical (inserting)
            {
                new_pool_runs.reserve(new_pool_runs.size() + private_new_pool_runs.size());
                new_pool_phases.reserve(new_pool_phases.size() + private_new_pool_phases.size());
                new_pool_energies.reserve(new_pool_energies.size() + private_new_pool_energies.size());

                new_pool_runs.insert(new_pool_runs.end(), private_new_pool_runs.begin(), private_new_pool_runs.end());
                new_pool_phases.insert(new_pool_phases.end(), private_new_pool_phases.begin(), private_new_pool_phases.end());
                new_pool_energies.insert(new_pool_energies.end(), private_new_pool_energies.begin(), private_new_pool_energies.end());
                pasos_simulados += private_pasos_simulados;
            }

            // Se limpian las listas de las siguientes generaciones (locales)
            private_new_pool_runs.clear();
            private_new_pool_phases.clear();
            private_new_pool_energies.clear();
            ...
            #pragma omp master
            {
                // El procesador principal usa la siguiente generación como la actual para la siguiente iteración
                pool_runs = new_pool_runs;
                pool_phase = new_pool_phases;
                pool_energies = new_pool_energies;

                // Se limpian las listas de las siguientes generaciones (globales)
                new_pool_runs.clear();
                new_pool_phases.clear();
                new_pool_energies.clear();
                ...
                std::cout << "Time of run: " << std::setprecision(2) << minutos_run << " min" << std::endl;
                std::cout << "Power=" << power << " W, run#: " << n << ", electrons alive:" << pool_runs.size() << std::endl;
                std::cout << "Pasos simulados run: " << aux_pasos_simulados << std::endl;
...
int run(){
    ...
    for (const auto power : RF_power)
    {
        ...
        // parallel = 1;
        if (parallel == 1)      std::tie(number_of_electrons, electrons_last_cycle) = run_n_electrons_parallel        (power, pool_runs, pool_phase, pool_energies);
        else if (parallel > 1)  std::tie(number_of_electrons, electrons_last_cycle) = run_n_electrons_parallel_thread (power, pool_runs, pool_phase, pool_energies);
        else {std::cerr << "18. Not a valid number of parallel units"<< std::endl; std::exit(1);}
