...
std::tuple<unsigned int, double, double, std::vector<std::vector<double>>> track_1_e_thread (unsigned long long int &private_pasos_simulados, double electron_energy = -1, double power = 1.0, double phase = 0.0, int face_i = -1, bool keep = false, bool show_in = false, std::shared_ptr<dolfin::BoundaryMesh> bmesh_in = nullptr,std::shared_ptr<dolfin::BoundingBoxTree> btree_in = nullptr, std::shared_ptr<dolfin::Function> campoEx_in = nullptr, std::shared_ptr<dolfin::Function> campoEy_in = nullptr, std::shared_ptr<dolfin::Function> campoEz_in = nullptr, std::shared_ptr<dolfin::BoundingBoxTree> tree_in = nullptr){
    ...
    double field_factor = std::sqrt(power);
    // std::cout << omp_get_thread_num() << " INSIDE 1" << std::endl;
    std::vector<std::vector<double>> trayectoria;
    std::vector<double> energia_electron;
    unsigned int collision = 0;
    double energy_collision;
    ...
    std::vector<double> U0  = result[1];
    std::vector<double> EX0 = result[2];
    // std::cout << omp_get_thread_num() << " INSIDE 2" << std::endl;
    // La velocidad inicial se calcula según la energía del electrón inicial y se guarda en V0
    ...
    std::vector<double> VX(V0);
    std::vector<double> X(X0);
    // std::cout << omp_get_thread_num() << " INSIDE 3" << std::endl;
    // Se calcula el tiempo máximo de simulación
    double t_max = N_cycles/RF_frequency;

    std::vector<double> P0 = {(gamma/speed_light_vacuum)*V0[0],(gamma/speed_light_vacuum)*V0[1],(gamma/speed_light_vacuum)*V0[2]};
    // std::cout << omp_get_thread_num() << " INSIDE 4" << std::endl;
    while (!ended)
    {        
        // Para hacer comparaciones de speed-ups, se guarda la cantidad de pasos simulados
        private_pasos_simulados++;
        // std::cout << omp_get_thread_num() << " INSIDE 5" << std::endl;
        // EX0 guarda la fuerza sufrida por el campo electromagnético en el punto del electrón, pero sin tener en cuenta otros factores
        EX0 = evaluate_campo_from_point_thread(X[0],X[1],X[2],campoEx_in, campoEy_in, campoEz_in);
        alpha = (w*t) + phase;
        // std::cout << omp_get_thread_num() << " INSIDE 6" << std::endl;
        // Se actualiza la fuerza sufrida por el electrón
        for (int i=0;i<3;i++) EX[i] = (EX0[i]*field_factor*std::cos(alpha));  
        ...
        // std::cout << omp_get_thread_num() << " INSIDE 7" << std::endl;
        max_gamma = std::max(gamma,max_gamma);
        ...
        // std::cout << omp_get_thread_num() << " INSIDE 8" << std::endl;
    } /* Termina el while not ended*/
    ...
    // std::cout << omp_get_thread_num() << " INSIDE 9" << std::endl;
    return std::make_tuple(collision,energy_collision,phase_collision,trayectoria);
}
 ...
 */
std::tuple<int, int> run_n_electrons_parallel (double power, Electron_list& original_pool_runs){

    // Mostrar y guardar estado inicial
    std::cout << "Power=" << power << "W, initial #electrons: " << original_pool_runs.electron_count << std::endl;
    std::ofstream logtime(logtime_name, std::ios::app);
    if (!logtime.is_open()) {std::cerr << "9. Error opening logtime file: " << logtime_name << std::endl; std::exit(1);}
    logtime << "Power=" << power << "W, initial #electrons: " << original_pool_runs.electron_count << std::endl;
    logtime.close();
    ...
    if (log_exec)
    {
        ...
        logfile << "Power=" << power << "W, initial #electrons: " << original_pool_runs.electron_count << std::endl;
        logfile.close();
    }
    ...
    #pragma omp parallel
    {
        #pragma omp single
        {
            tnr = omp_get_num_threads();
        }
    }

    Electron_list pool_runs[tnr];
    Electron_list new_pool_runs[tnr];
    ...
    #pragma omp parallel
    {
        ...
        while (!ended)
        {
            ...
            { 
                ...
                if (original_pool_runs.electron_count == 0)
                {
                    ended = true;
                } else {
                    // Repartir electrones entre los threads
                    int total_electrons = original_pool_runs.electron_count;
                    for (int k=0;k<total_electrons;k++){
                        add_electron(pool_runs[k%tnr],get_electron(original_pool_runs));
                    }
                }
            } 
            ...
            if (pool_runs[tid].electron_count > 0) // Comprueba que haya algo en la lista
            {
                std::shared_ptr<Electron> actual_electron = std::make_shared<Electron>(*get_electron(pool_runs[tid]));
                while (actual_electron->face != -1)
                {
                    // std::cout << tid << " inside while" << std::endl;
                    ...
                    if (face > 0) 
                    {
                        ...
                        for (int j=0;j<n_e;j++)
                        {
                            ...
                            add_electron(new_pool_runs[tid],new_e); 
                        }
                    }

                    // Se busca el siguiente electrón de la lista
                    actual_electron = std::make_shared<Electron>(*get_electron(pool_runs[tid]));
                }
            }
            ...
            { 
                ...
                if (minutos_run >= 1) logtime << "Completed run " << n << ", time: " << minutos_run << " min, electrons alive: " << original_pool_runs.electron_count << std::endl;
                else logtime << "Completed run " << n << ", time: " << minutos_run*60 << " sec, electrons alive: " << original_pool_runs.electron_count << std::endl;
                logtime.close();

                // Mostrar tiempo de una 'run'
                std::cout << "Time of run: " << std::setprecision(2) << minutos_run << " min" << std::endl;
                std::cout << "Power=" << power << " W, run#: " << n << ", electrons alive:" << original_pool_runs.electron_count << std::endl;
                ...
                if (log_exec)
                {
                    ...
                    logfile << "Completed secondary run #: " << n << ", power=" << power << " W, electrons alive:" << original_pool_runs.electron_count << std::endl << std::endl;
                    logfile.close();
                }

                // Actualizar número total de electrones simulados y generaciones
                number_electrons = number_electrons + original_pool_runs.electron_count;
                n = n+1;
    ...
    return std::make_tuple(number_electrons,original_pool_runs.electron_count);
}
